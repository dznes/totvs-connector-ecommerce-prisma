...
export interface ProductsRepository {
  ...
  searchMany(query: string, productCode: string, integrationCode: string, productTitle: string, page: number, perPage: number): Promise<ProductWithSkuAndVariants[]>
  listRecentProducts(): Promise<Product[] | null>
  count(query: string, productCode: string, integrationCode: string, productTitle: string): Promise<number>
