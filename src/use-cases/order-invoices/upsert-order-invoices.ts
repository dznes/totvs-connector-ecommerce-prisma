  ...
  async execute({
    ...
    if (old_order_invoice) {
      await this.orderInvoicesRepository.update({
        ...
        tracking_code:
          order_invoice.trackingCode ?? old_order_invoice.tracking_code,
        shipping_company_name:
          order_invoice.shippingCompanyName ??
          old_order_invoice.shipping_company_name,
        ...
        gross_weight:
          order_invoice.grossWeight ?? old_order_invoice.gross_weight,
        package_number:
          order_invoice.packageNumber ?? old_order_invoice.package_number,
        quantity: order_invoice.quantity ?? old_order_invoice.quantity,
        discount_percentage:
          order_invoice.discountPercentage ??
          old_order_invoice.discount_percentage,
        additional_value: convertToDecimal(
          order_invoice.additionalValue ?? old_order_invoice.additional_value,
        ),
        product_value: convertToDecimal(
          order_invoice.productValue ?? old_order_invoice.product_value,
        ),
        shipping_value: convertToDecimal(
          order_invoice.shippingValue ?? old_order_invoice.shipping_value,
        ),
        insurance_value: convertToDecimal(
          order_invoice.InsuranceValue ?? old_order_invoice.insurance_value,
        ),
        ipi_value: convertToDecimal(
          order_invoice.ipiValue ?? old_order_invoice.ipi_value,
        ),
        total_value: convertToDecimal(
          order_invoice.totalValue ?? old_order_invoice.total_value,
        ),
        ...
        transaction_date:
          order_invoice.transactionDate ?? old_order_invoice.transaction_date,
      ...
      try {
        await this.orderInvoicesRepository.create({
          ...
          transaction_branch_code:
            order_invoice.transactionBranchCode.toString(),
          transaction_code: order_invoice.transactionCode.toString(),
        ...
        if (
          ...
          console.error(
            `Order Invoice with code ${order_invoice.code} already exists.`,
          )
