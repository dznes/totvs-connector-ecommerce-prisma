  ...
  app.get('/api/dashboard/sales-30-days-faturado', async (request, reply) => {
    ...
    try {
        ...
        orderItems.forEach(item => {
            ...
            const discount = item.discount_percentage ? item.discount_percentage / 100 : 0;
            const finalPrice = price * (1 - discount) * quantity;

            const date = item.order.totvs_creation_date.toISOString().split('T')[0]; // Extract only the date part (YYYY-MM-DD)
