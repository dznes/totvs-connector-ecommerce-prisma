...
export async function upsertTotvsClassifications(request: FastifyRequest, reply: FastifyReply) {
  ...
  try {
    ...
    return reply.status(201).send({ message: 'Classifications upserted successfully' })
  } catch (err) {
    console.log(err)
