...
export async function calculateDeliveryPriceAndDate(
  ...
  const { codigoServico, cepDestino, cepOrigem, pesoObjeto, tipoObjeto, comprimento, largura, altura, diametro } =
  bodySchema.parse(request.body)
  try {

    const { pcFinal } = await calculatePrice({ codigoServico, cepDestino, cepOrigem, pesoObjeto, tipoObjeto, comprimento, largura, altura, diametro })
    const { prazoEntrega, dataMaxima } = await calculateDeliveryDate({ codigoServico, cepDestino, cepOrigem })

    return reply.status(201).send({ price: pcFinal, deliveryDate: prazoEntrega, maxDate: new Date(dataMaxima) })
  } catch (err) {
    if (err) {
