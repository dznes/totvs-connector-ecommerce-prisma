...
import { makeUpdateSkuCostsUseCase } from '@/use-cases/factories/skus/make-update-sku-costs-use-case'
...
export async function skuCostsBackup(_: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    return reply.status(500).send({ error: 'Failed to fetch SKU costs' })
  }
}
