import { fetchToken, getWholesaleClients } from '@/http/lib/totvs'
 ...
 */
export async function wholesaleUsersBackup(
  _: FastifyRequest,
  reply: FastifyReply,
) {
  try {
    ...
    while (!isLastPage) {
      ...
      const { items, hasNext } = await getWholesaleClients({
    ...
    return reply
      .status(500)
      .send({ error: 'Failed to fetch wholesale users details' })
  }
}
